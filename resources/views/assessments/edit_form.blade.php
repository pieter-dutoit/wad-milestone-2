    ...
    <div class='table-card'>
        ...
        <form action="{{ url("assessments/$assessment->id") }}" method="post">
            ...
            <div class="input-wrapper">
                ...
                <textarea rows="7" cols="30" class="form-control" name="instruction" id="instruction">{{ old('instruction') ?? $assessment->instruction }}</textarea>
                <ul>
            ...
            <div class="input-wrapper">
                ...
                <input class="form-control" type="number" name="num_reviews" id="num_reviews"
                    value="{{ old('num_reviews') ?? $assessment->num_reviews }}">
                <ul>
            ...
            <div class="input-wrapper">
                ...
                <input class="form-control" type="number" name="max_score" id="max_score"
                ...
                @foreach ($reviewTypes as $index => $reviewType)
                    <div>
                        <input class="form-check-input" type="radio" name="type_id" id="{{ $reviewType->id }}"
                            value="{{ $reviewType->id }}"
                            {{ ((old('type_id') ?? $assessment->type->id) == $reviewType->id ? 'checked' : $index == 0) ? 'checked' : '' }}>
